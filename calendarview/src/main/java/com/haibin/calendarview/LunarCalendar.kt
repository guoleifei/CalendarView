...
internal object LunarCalendar {
     ...
     * 闰九月 农历正月初一对应公历1月31号
     */
    private val LUNAR_INFO = intArrayOf(0x84B6BF, /*1900*/
    ...
    private fun daysInLunarMonth(year: Int, month: Int): Int {
        return if (LUNAR_INFO[year - CustomCalendarViewDelegate.MIN_YEAR] and (0x100000 shr month) == 0)
            29
        else
            30
    }
    ...
    private fun getSolarTerm(year: Int, month: Int, day: Int): String {
        ...
        val text = String.format("%s%s", year, getString(month, day))

        return solarTerm?.singleOrNull { it.contains(text) }?.replace(text, "") ?: ""
    }
    ...
    fun getLunarText(year: Int, month: Int, day: Int): String {
        ...
        return if (!TextUtils.isEmpty(festival)) festival else numToChinese(lunar[1], lunar[2], lunar[3])
    }
    ...
    fun setupLunarCalendar(calendar: Calendar) {
        ...
        calendar.isWeekend = Util.isWeekend(calendar)
        calendar.week = Util.getWeekFormCalendar(calendar)

        val lunarCalendar = Calendar()
        calendar.lunarCakendar = lunarCalendar
        val lunar = LunarUtil.solarToLunar(year, month, day)
        lunarCalendar.year = lunar[0]
        lunarCalendar.month = lunar[1]
        lunarCalendar.day = lunar[2]
        calendar.isLeapYear = Util.isLeapYear(year)
        if (lunar[3] == 1) {//如果是闰月
            calendar.leapMonth = lunar[1]
            lunarCalendar.leapMonth = lunar[1]
        }
        val solarTerm = getSolarTerm(year, month, day)
        val gregorian = gregorianFestival(month, day)
        val festival = getTraditionFestival(lunar[0], lunar[1], lunar[2])
        ...
        if (!TextUtils.isEmpty(solarTerm)) {
            ...
            calendar.lunar = numToChinese(lunar[1], lunar[2], lunar[3])
        }
        lunarCalendar.lunar = calendar.lunar
    }
